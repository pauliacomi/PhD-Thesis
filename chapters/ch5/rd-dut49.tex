...
differential enthalpy of adsorption. \autoref{dut:fig:dut-49-butane} shows
...
reason behind the dissimilar behaviour may be found in the contribution
...
instead accessing an intermediate phase (or \textit{ip}), 
with the \textit{a} lattice parameter 5--7\% smaller than 
the \textit{op} phase rather than 24\% in the \textit{cp} phase.
...
Instead, as the net change in adsorbed amount between the two
...
of Uncertainty in Measurements (GUM). The method is detailed 
...
\begin{figure}[htb]
    ...
    Continuous lines represent the adsorption 
    branch while dotted lines represent the desorption branch.
...
However, when observing \gls{dh} as a function of 
