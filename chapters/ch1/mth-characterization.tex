...
will not give a realistic surface area. Furthermore, the cross-sectional
...
finds the optimum pressure range and returns the results as a
...
\begin{python}[caption={Generating a t-plot},label={pyg:lst:tplot}]
...
To generate an \(\alpha_s\)-plot in \texttt{pyGAPS}, both an analysis isotherm
...
The reference material area can be specified by using the \inline{reference_area}
