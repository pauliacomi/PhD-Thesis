...
\begin{equation}\label{pyg:eqn:kelvin}
	\ln\Big(\frac{p}{p_0}\Big) = -\frac{\gamma M_m}{\rho_l R_gT}\frac{2 \cos\theta}{r}
\end{equation}
%
...
\begin{multline}\label{pyg:eqn:hk}
	R_g T ln(p/p_0) =   N_A\frac{n_{ads} A_a + n_A A_A}{2 \sigma^{4}(l-d)}
...
\(n_{ads}\) is number of molecules of adsorbent
...
%
\begin{align}
	A_a & = \frac{6m_{e^-}c^2\alpha_a\alpha_A}{\alpha_a/\varkappa_a + \alpha_A/\varkappa_A}
	%
	\intertext{and}
	%
	A_a & = \frac{3m_{e^-}c^2\alpha_A\varkappa_A}{2}
\end{align}
%
where \(m_{e^-}\) is the mass of an electron, \(\alpha_a\) and \(\alpha_A\) are
