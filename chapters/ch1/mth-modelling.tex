...
We can then apply the Langmuir theory for each layer. \todo{better structure}

\begin{align}
    k_{a_1} p \theta_0 &= k_{d_1} \theta_1 \exp{\Big(-\frac{E_1}{RT}\Big)} \\
    k_{a_2} p \theta_1 &= k_{d_2} \theta_2 \exp{\Big(-\frac{E_L}{RT}\Big)} \\
    \vdots \nonumber \\
    k_{a_i} p \theta_{i-1} &= k_{d_i} \theta_i \exp{\Big(-\frac{E_L}{RT}\Big)}
\end{align}
