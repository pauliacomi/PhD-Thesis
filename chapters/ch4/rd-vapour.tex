...
On the parent UiO-66(Zr), the water isotherm 
(\autoref{shaping:fgr:wateruio66}) is characterized by a slow uptake 
...
The methanol isotherm (\autoref{shaping:fgr:methanoluio66})
has the same features as the water
...
The water isotherm on the powder MIL-100(Fe) material 
(\autoref{shaping:fgr:watermil100}) shows a more 
...
The methanol isotherm (\autoref{shaping:fgr:methanolmil100}) on 
the same parent MOF still shows 
...
\SI{5}{\milli\mol\per\gram} smaller than the PVA and powder
...
In terms of the pore volume, the MRA isotherms 
...
``gating'' effect seen with butane adsorption on the PVA pellets
...
On the last MOF powder, the water isotherm 
(\autoref{shaping:fgr:watermil127}) shows a hydrophilic
...
A slight hysteresis curve is observed, likely due to the 
fine particulate nature of the sample.
...
in the order of powder > PVA > alumina. The binder has not affected
